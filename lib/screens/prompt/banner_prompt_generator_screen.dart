...
const _backgroundOptions = [
  _BgOption(label: 'Gradien Neon Biru-Magenta', value: 'blue to magenta neon gradient background'),
  _BgOption(label: 'Putih Bersih Minimalis', value: 'clean white background, minimal design'),
  _BgOption(label: 'Hitam Premium Elegan', value: 'black premium dark background with carbon accents'),
  _BgOption(label: 'Gradien Emas-Kuning Menyala', value: 'gold to yellow glossy gradient background'),
  _BgOption(label: 'Gradien Hijau-Teal Segar', value: 'green to teal gradient fresh background'),
  _BgOption(label: 'Abu-abu Netral Soft', value: 'soft neutral gray studio background'),
  _BgOption(label: 'Retro Pop Art Merah-Kuning', value: 'retro yellow and red pop art background'),
];

const _layoutOptions = [
  'Spanduk Horizontal (3:1) - Canvas: 3000 × 1000 px',
  'Spanduk Panjang (4:1) - Canvas: 4000 × 1000 px',
  'Vertical X-Banner (2:3) - Canvas: 1200 × 1800 px',
  'Square Banner (1:1) - Canvas: 1080 × 1080 px',
  'Billboard Baliho Raksasa (16:9) - Canvas: 1920 × 1080 px',
];

const _contentTypesBanner = [
  'Spanduk Toko / Booth Fisik',
  'Spanduk Promo / Grand Opening',
  'Baliho Media Jalan Raya',
  'Standing X-Banner / Roll Banner',
  'Banner Event / Kegiatan Sosial',
];
...
class BannerPromptGeneratorScreen extends ConsumerStatefulWidget {
  const BannerPromptGeneratorScreen({super.key});

  @override
  ConsumerState<BannerPromptGeneratorScreen> createState() => _BannerPromptGeneratorScreenState();
}

class _BannerPromptGeneratorScreenState extends ConsumerState<BannerPromptGeneratorScreen> {
  ...
  String _selectedContentType = _contentTypesBanner.first;
  ...
  Future<void> _pickImage() async {
    if (_referenceImages.length >= 3) {
      ToastMessage.showWarning(context, 'Maksimal 3 foto referensi.');
      return;
    }
  ...
  Future<void> _generate() async {
    ...
    try {
      ...
      ToastMessage.showError(context, 'Gagal mengunggah foto referensi: $e');
    ...
    final designStyleString = '$_selectedDesignStyleName | Latar: $bgLabel | Dimensi: $_selectedLayout';

    await ref.read(promptProvider.notifier).generateBannerPrompt(
      title: _titleController.text.trim(),
      contentType: _selectedContentType,
      businessType: _businessTypeController.text.trim(),
      designStyle: designStyleString,
  ...
  Widget build(BuildContext context) {
    ...
    final accentColor = const Color(0xFF0D9488); // Teal style accent color

    return Stack(
      children: [
        Scaffold(
          appBar: AppBar(
            title: const Text('Spanduk & Baliho Bisnis', style: TextStyle(fontWeight: FontWeight.bold)),
            elevation: 0,
              ...
              : SafeArea(
                ...
                const SizedBox(height: 12),
                          ...
                          CustomTextField(
                            ...
                            labelText: 'Nama Bisnis / Brand Utama',
                            hintText: 'Contoh: Doorsmeer Auto Clean',
                            validator: (val) => val == null || val.trim().isEmpty ? 'Nama Bisnis wajib diisi' : null,
                          ),
                          const SizedBox(height: 16),

                          CustomTextField(
                            controller: _businessTypeController,
                            labelText: 'Jenis Usaha / Bidang Bisnis',
                            hintText: 'Contoh: Doorsmeer Mobil & Motor, Kedai Kopi, Laundry',
                            validator: (val) => val == null || val.trim().isEmpty ? 'Jenis Usaha wajib diisi' : null,
                          ),
                          const SizedBox(height: 16),

                          CustomTextField(
                            controller: _descController,
                            labelText: 'Deskripsi & Layanan Utama',
                            hintText: 'Tulis layanan atau promo banner di sini (Contoh: Cuci hidrolik salju, poles body, vacum interior)',
                            maxLines: 3,
                            validator: (val) => val == null || val.trim().isEmpty ? 'Layanan utama wajib diisi' : null,
                          ),
                          const SizedBox(height: 16),

                          CustomTextField(
                            controller: _contactController,
                            labelText: 'Informasi Kontak & Alamat (Opsional)',
                            hintText: 'Contoh: Hubungi 0812-3456-7890 | Jl. Raya Clean No. 10',
                          ),
                          const SizedBox(height: 20),

                          // Image picker
                          Text(
                            'Foto Referensi Layout / Desain (Maks 3)',
                          ...
                          Container(
                            ...
                            child: DropdownButtonHideUnderline(
                              child: DropdownButton<String>(
                                ...
                                items: _contentTypesBanner.map((String value) {
                          ...
                          Text(
                            'Konsep Latar Belakang Spanduk',
                            style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold),
                          ),
                          const SizedBox(height: 8),
                          _buildBackgroundDropdown(isDark),
                          const SizedBox(height: 20),

                          _buildCaptionToggle(theme, accentColor),
  ...
  Widget _buildBackgroundDropdown(bool isDark) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
      decoration: BoxDecoration(
        color: isDark ? AppColors.darkSurface : Colors.grey[100],
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          items: _backgroundOptions.map((bg) {
            return DropdownMenuItem<String>(
              value: bg.value,
              child: Text(bg.label, overflow: TextOverflow.ellipsis),
            );
          }).toList(),
          onChanged: (val) {
  ...
  Widget _buildCaptionToggle(ThemeData theme, Color accentColor) {
    return Container(
      decoration: BoxDecoration(
        ...
        child: Row(
          ...
          children: [
            Column(
              ...
              children: [
                Text(
                  'Buat Caption & Hashtag',
                  style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold),
                ),
                Text(
                  'Dapatkan caption promo Instagram/TikTok gratis.',
  ...
  Widget _buildLoadingSteps(ThemeData theme) {
    ...
    String activeText = 'Menyiapkan deskripsi & referensi spanduk...';
    if (_referenceImages.isNotEmpty && _uploadedImageUrls.length == _referenceImages.length) {
      activeText = 'Menganalisis gambar referensi desain...';
    } else if (_referenceImages.isNotEmpty && _uploadedImageUrls.length < _referenceImages.length) {
      activeText = 'Mengunggah gambar referensi layout...';
    }
    ...
    return Column(
      children: [
        Row(
          ...
          children: [
            const Icon(Icons.brush_rounded, size: 16, color: Color(0xFF0D9488)),
            const SizedBox(width: 8),
            Expanded(
              child: Text(
                activeText,
        ...
        const SizedBox(height: 8),
        Text(
          'Proses ini memerlukan waktu 10-15 detik karena LLM sedang menyusun aturan safe area cetak.',
